...
public class AuthService {
    ...
    /**
     * Registers a new buyer. The role is hard-coded to {@link Role#BUYER}:
     * any role hint coming from the client is irrelevant because the DTO
     * does not even carry that field. This is the application-level
     * enforcement of RF-01 (least privilege on public sign-up).
     */
    @Transactional
    ...
    /**
     * Authenticates an existing user. We delegate credential checking
     * to Spring's {@link AuthenticationManager}, which goes through the
     * configured {@code DaoAuthenticationProvider} and {@code BCryptPasswordEncoder}.
     * Failed authentication bubbles up as a {@code BadCredentialsException}
     * and is mapped to HTTP 401 by {@link com.motoshop.api.web.GlobalExceptionHandler}.
     */
    public AuthResponse login(LoginRequest req) {
        ...
        String token = jwtService.issueToken(principal.getId(), principal.getUsername(), principal.getRole());
