 ...
 *
 * <p>The token carries the user id as subject, the email as a custom claim and the role as another
 * custom claim. We deliberately keep the payload minimal: the resource server resolves the user
 * from the id on every request, so we never trust client-side state.
 */
@Service
public class JwtService {
  ...
  /**
   * Parses and validates a token. Returns an empty optional if the token is malformed, expired or
   * signed with a different key. Callers MUST treat an empty optional as an authentication failure.
   */
