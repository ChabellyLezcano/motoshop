...
/**
 * Translates uncaught exceptions to JSON error responses with a stable shape: {@code status, error,
 * message, path, timestamp, [fieldErrors]}. The 401/403 paths are handled directly by Spring
 * Security's entry points; this advice covers everything else that escapes a controller.
 */
...
public class GlobalExceptionHandler {
  ...
  /**
   * Both bad password and unknown user must answer with the SAME error, so callers cannot tell
   * whether the email exists. This is a small but important hardening against account enumeration.
   */
  @ExceptionHandler({BadCredentialsException.class, UsernameNotFoundException.class})
  public ResponseEntity<Map<String, Object>> handleBadCredentials(
      Exception ex, HttpServletRequest req) {
    return body(HttpStatus.UNAUTHORIZED, "Unauthorized", "Invalid email or password", req, null);
  }
  ...
  public ResponseEntity<Map<String, Object>> handleMotorcycleNotFound(
      MotorcycleNotFoundException ex, HttpServletRequest req) {
