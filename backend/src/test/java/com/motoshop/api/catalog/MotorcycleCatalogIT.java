...
class MotorcycleCatalogIT extends PostgresIntegrationTest {
    ...
    void filterByLicenseA2() throws Exception {
        ...
        content.forEach(item ->
                assertThat(item.get("license").asText()).isEqualTo("A2"));
    }
    ...
    void adminCreatesAndItAppears() throws Exception {
        ...
        var create = new CreateMotorcycleRequest(
                "TestMaker", "QA-Special", 500, 2024, 555000L, 4,
                "A bike that only exists in tests.",
                50, 50, 180, EngineType.PARALLEL_TWIN, Cooling.LIQUID,
                170, 800, new BigDecimal("14.0"), "Hi-Vis Yellow",
                Category.NAKED, LicenseType.A2, 6, true, false
        );
        ...
        var created = http.exchange(
                ...
                new HttpEntity<>(create, h), String.class);
    ...
    void invalidCreateReturns400() throws Exception {
        ...
        var resp = http.exchange(
                "/api/motorcycles", HttpMethod.POST,
                new HttpEntity<>(invalid, h), String.class);
    ...
    private String loginAsAdmin() {
        var resp = http.postForEntity("/api/auth/login",
