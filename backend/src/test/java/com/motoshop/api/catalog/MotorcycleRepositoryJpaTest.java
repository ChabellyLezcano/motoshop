...
/**
 * Persistence-layer tests using H2 and Spring Data's {@code @DataJpaTest} slice. We rely on
 * AUTO-detected H2 since the "test" profile already routes the datasource there.
 *
 * <p>The goal is to lock the JpaSpecifications behaviour: each filter narrows the result set as we
 * expect, and composing them with AND intersects correctly.
 */
@DataJpaTest
...
class MotorcycleRepositoryJpaTest {
  ...
  void filtersCombineWithAnd() {
    var spec =
        MotorcycleSpecifications.allOf(
    ...
    assertThat(repo.findAll(spec, PageRequest.of(0, 10)))
        .singleElement()
        .extracting(Motorcycle::getModel)
        .isEqualTo("390 Duke");
  }
  ...
  private static Motorcycle bike(
      String brand, String model, Category cat, LicenseType lic, long priceCents, int stock) {
